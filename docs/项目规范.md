...
- **React 组件文件**：使用 **PascalCase**（符合 React 社区习惯）
  - ✅ 正确：`NavBar.tsx`, `Provider.tsx`, `SelectInput.jsx`
  - ❌ 错误：`nav-bar.tsx`, `provider.tsx`
  
- **第三方 UI 库组件**：保持原样（如 shadcn/ui 使用 kebab-case）
  - ✅ 正确：`button.jsx`, `dropdown-menu.jsx`（shadcn/ui 默认命名）
  
- **Next.js 页面文件**：使用小写（Next.js 约定）
  - ✅ 正确：`page.tsx`, `layout.tsx`（Next.js 约定）
